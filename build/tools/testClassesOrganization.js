...
const classesBase = classes.filter((c) => c.tipo === "classe_base");
const variantes = classes.filter((c) => c.tipo === "variante");
...
variantes.forEach((v) => {
    if (v.classe_base) {
        const baseExists = classesBase.some((c) => c.id === v.classe_base);
        if (!baseExists) {
            issues.push(`Variante "${v.nome}" referencia classe base inexistente: ${v.classe_base}`);
        }
    }
});
if (issues.length > 0) {
