...
function saveAcervo(data) {
    writeFileSync(ACERVO_PATH, JSON.stringify(data, null, 2), "utf-8");
}
function showStats(data) {
    const poderesTotal = Object.values(data.poderes_gerais || {}).reduce((sum, arr) => sum + (arr?.length || 0), 0);
    const itensTotal = Object.values(data.itens || {}).reduce((sum, arr) => sum + (arr?.length || 0), 0);
    const deusesTotal = (data.deuses_maiores?.length || 0) +
    ...
    console.log(`   ├─ Racial: ${data.poderes_gerais?.racial?.length || 0}, Combate: ${data.poderes_gerais?.combate?.length || 0}, Destino: ${data.poderes_gerais?.destino?.length || 0}`);
    console.log(`   └─ Magia: ${data.poderes_gerais?.magia?.length || 0}, Tormenta: ${data.poderes_gerais?.tormenta?.length || 0}, Concedido: ${data.poderes_gerais?.concedido?.length || 0}`);
    console.log(`📦 Itens: ${itensTotal}`);
    console.log(`   ├─ Mundanos: ${data.itens?.mundanos?.length || 0}, Consumíveis: ${data.itens?.consumiveis?.length || 0}`);
    console.log(`   └─ Mágicos: ${data.itens?.magicos?.length || 0}, Aprimorados: ${data.itens?.aprimorados?.length || 0}`);
    console.log(`🕯️  Divindades: ${deusesTotal}`);
    console.log(`   ├─ Maiores: ${data.deuses_maiores?.length || 0}, Menores: ${data.deuses_menores?.length || 0}, Servidores: ${data.deuses_servidores?.length || 0}`);
...
function listItems(category, data, limit = 50) {
    ...
    if (Array.isArray(items)) {
        ...
        if (items.length > limit)
            console.log(`... e mais ${items.length - limit} itens`);
    }
    else {
...
function searchItems(query, data) {
    ...
    function matches(item, categoryName) {
        // Check type filter
    ...
    function searchArray(arr, categoryName) {
        arr.forEach((item) => {
    ...
    if (results.length > 20)
        console.log(`... e mais ${results.length - 20} resultados`);
}
function deleteItem(category, itemName, data) {
    const targets = [];
    function removeFromArray(arr, categoryName) {
        const idx = arr.findIndex((item) => (item.nome || item.name || item.id || "").toLowerCase() === itemName.toLowerCase());
        if (idx !== -1) {
    ...
    let found = false;
    if (category === "racas")
        found = removeFromArray(data.racas, "Raças");
    else if (category === "classes")
        found = removeFromArray(data.classes, "Classes");
    else if (category === "classes_alternativas")
        found = removeFromArray(data.classes_alternativas, "Classes Alternativas");
    else if (category === "origens")
        found = removeFromArray(data.origens, "Origens");
    ...
    else if (category === "distincoes")
        found = removeFromArray(data.distincoes, "Distinções");
    else if (category === "bases")
        found = removeFromArray(data.bases, "Bases");
    else if (category === "dominios")
        found = removeFromArray(data.dominios, "Domínios");
        ...
        switch (cmd) {
            ...
            case "delete":
                if (args.length >= 2) {
                    ...
                    const confirm = await promptUser(`⚠️ Confirma remoção de "${name}" da categoria "${cat}"? (s/n): `);
