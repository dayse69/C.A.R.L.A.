...
import { getAllClassesExtended, getAllRacesExtended, getAvailableCategories, } from "./compendiumService.js";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const syncStates = new Map();
function key(categoria, name) {
    return `${categoria}:${name.toLowerCase()}`;
}
function getPersistPath(guildId) {
...
function loadPersistedState(guildId) {
    try {
...
export async function ensureChannels(guild, baseName = "acervo") {
    const categoryName = "📚 acervo";
    const desiredCategories = getAvailableCategories();
    // Encontrar ou criar categoria principal
    const existingCategory = guild.channels.cache.find((c) => c.type === ChannelType.GuildCategory && c.name === categoryName);
    ...
    for (const catName of desiredCategories) {
        ...
        const found = guild.channels.cache.find((c) => c.type === ChannelType.GuildText && c.name === channelName);
        if (found) {
    ...
    const textChannelsInCategory = guild.channels.cache.filter((c) => c.type === ChannelType.GuildText &&
...
export async function syncAcervo(guild, opts) {
    ...
    const publish = async (categoria, channel, items) => {
...
export function getSyncState(guildId) {
