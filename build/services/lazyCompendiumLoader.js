...
class LazyCompendiumLoader {
    ...
    getClasses() {
        ...
        console.log(`[LazyLoader] Classes carregadas: ${this.cache.classes?.length || 0} itens em ${loadTime.toFixed(2)}ms`);
        return this.cache.classes || [];
    }
    /**
    ...
    getRacas() {
        ...
        console.log(`[LazyLoader] Raças carregadas: ${this.cache.racas?.length || 0} itens em ${loadTime.toFixed(2)}ms`);
        return this.cache.racas || [];
    }
    /**
    ...
    getStats() {
        return {
            ...
            avgLoadTime: this.stats.length > 0
                ? (this.stats.reduce((sum, s) => sum + s.loadTime, 0) / this.stats.length).toFixed(2)
                : "0",
        };
    }
}
