...
import { calcularModificador, formatarModificador, } from "../../services/fichaService.js";
import { COLORS, EMOJIS } from "../../utils/constants.js";
/**
 * ABA 1: 🧿 Geral - Identificação e atributos (Tema: A Sombra Escaroluz)
 */
export function criarEmbedGeralFicha(character) {
    const mods = {
    ...
    const classesText = classes.length > 0
        ...
        : character.classe || "Classe desconhecida";
    const nivelTotal = character.nivelTotal || character.nivel || 1;
    const aparencia = character.historia?.aparencia ||
        character.notas ||
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`**${character.raca}** • ${classesText} • Nível **${nivelTotal}**\n\n` +
        `*${aparencia}*`)
        .addFields({
        name: `💠 ATRIBUTOS`,
 ...
 */
export function criarEmbedCombateFicha(character) {
    // Verificação defensiva para dados antigos
    const classes = Array.isArray(character.classes) ? character.classes : [];
    const classesText = classes.length > 0
        ? classes.map((c) => `${c.nome} ${c.nivel}`).join(" / ")
        : character.classe || "Classe desconhecida";
    const pv = character.recursos?.pv || { atual: 0, maximo: 12, temporario: 0 };
    const pm = character.recursos?.pm || { atual: 0, maximo: 10 };
    const defesa = character.recursos?.defesa ?? 10;
    const resistencias = character.recursos?.resistencias || {
        fortitude: 0,
        reflexos: 0,
        vontade: 0,
    };
 ...
 */
export function criarEmbedPericiasFicha(character) {
 ...
 */
export function criarEmbedPoderesFicha(character) {
 ...
 */
export function criarEmbedMagiasFicha(character) {
    ...
    if (magias.length === 0) {
        ...
        magiasText = magias
            ...
            .map((magia, i) => `${i + 1}. 🔮 **${magia.nome}** (Círculo ${magia.circulo}) — \`${magia.custoPM} PM\``)
            .join("\n");
    }
    const pm = character.recursos?.pm || { atual: 0, maximo: 10 };
    const pmPercentual = Math.round((pm.atual / pm.maximo) * 10);
 ...
 */
export function criarEmbedInventarioFicha(character) {
    ...
    if (inventario.length === 0) {
        ...
        inventarioText = inventario
            .map((item) => {
            const raridadeEmoji = item.raridade === "Lendário"
                ? "🌠"
                : item.raridade === "Raro"
                    ? "💎"
 ...
 */
export function criarEmbedNotasFicha(character) {
    const historia = character.historia;
    const anotacoes = historia.anotacoes ||
 ...
 */
export function criarEmbedFichaPrincipal(character) {
    return criarEmbedGeralFicha(character);
}
/**
 * Cria embed de inventário
 */
export function criarEmbedInventario(character) {
    const inventarioText = character.inventario.length === 0
        ? "Inventário vazio"
        : character.inventario
            .map((item) => `${EMOJIS.PLUS} **${item.nome}** (x${item.quantidade})`)
            .join("\n");
    return new EmbedBuilder()
 ...
 */
export function criarEmbedPericias(character) {
 ...
 */
export function criarEmbedResumoFicha(character) {
    const classes = Array.isArray(character.classes) ? character.classes : [];
    const classesText = classes.length > 0
        ? classes.map((c) => `${c.nome} ${c.nivel}`).join(" / ")
        : character.classe || "Classe desconhecida";
 ...
 */
export function criarEmbedStatusFicha(character) {
 ...
 */
export function criarEmbedXPFicha(character) {
    const xp = character.experiencia || { atual: 0, proximo: 1000, total: 0 };
    const nivelTotal = character.nivelTotal || 1;
 ...
 */
export function criarEmbedReacoesFicha(character) {
    const reacoes = character.reacoes || [];
    let reacoesText = "";
    if (reacoes.length === 0) {
        reacoesText =
 ...
 */
export function criarEmbedConfirmacao(titulo, descricao, sucesso = true) {
    return new EmbedBuilder()
        .setColor((sucesso ? COLORS.SUCCESS : COLORS.ACCENT))
 ...
 * Cria embed de erro
 */
export function criarEmbedErro(titulo, mensagem) {
    return new EmbedBuilder()
