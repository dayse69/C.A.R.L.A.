...
import { ActionRowBuilder, ApplicationCommandOptionType, ApplicationCommandType, ModalBuilder, PermissionFlagsBits, TextInputBuilder, TextInputStyle, } from "discord.js";
createCommand({
    ...
    async autocomplete(interaction) {
        ...
        const filtered = categorias.filter((c) => c.toLowerCase().includes(focused.value.toLowerCase()));
    ...
    async run(interaction) {
        ...
        const detalhesField = new TextInputBuilder()
            ...
            .setRequired(false)
            .setMaxLength(2000);
        modal.addComponents(new ActionRowBuilder().addComponents(descField), new ActionRowBuilder().addComponents(detalhesField));
