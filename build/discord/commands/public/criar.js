...
import { ActionRowBuilder, ApplicationCommandOptionType, ApplicationCommandType, EmbedBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, } from "discord.js";
...
import { converterParaCharacter, extrairTextoPDF, parsearFichaPDF, } from "../../../services/pdfParserService.js";
...
createCommand({
    ...
    async run(interaction) {
        try {
            ...
            console.log(`[criar] command invoked by user=${interaction.user?.id} guild=${interaction.guild?.id} nome=${nome}`);
            ...
            if (pdfAttachment) {
                // Validar tipo
                const isPdf = (pdfAttachment.contentType || "").includes("pdf") ||
                ...
                console.log(`[criar] importando PDF da ficha: ${pdfAttachment.name} url=${pdfAttachment.url}`);
                // Baixar para arquivo temporário
                const tmpDir = os.tmpdir();
                const tmpFile = path.join(tmpDir, `${Date.now()}-${sanitizeForCustomId(pdfAttachment.name || "ficha")}.pdf`);
                ...
                if (nome)
                    fichaExtraida.nome = nome;
                if (raca)
                    fichaExtraida.raca = raca;
                if (classe)
                    fichaExtraida.classe = classe;
                const character = converterParaCharacter(fichaExtraida, interaction.user.id);
                character.origem = origem || undefined;
                character.categoria = "ficha_personagem";
                const existente = await CharacterRepository.findByUserAndName(interaction.user.id, character.nome);
                if (existente) {
                ...
                // Exibir card e menu
                const embed = criarProfileCard({ ...character, _id: id });
            ...
            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId(`criar_tipo_select/${safeNome}/${safeOrigem}/${safeRaca}/${safeClasse}`)
