    ...
    async run(interaction) {
        ...
        try {
            ...
            if (nomePersonagem) {
                ...
                personagem = await CharacterRepository.findByUserAndName(interaction.user.id, nomePersonagem);
                if (!personagem) {
                    const embedErro = criarEmbedErro("Personagem não encontrado", `Você não possui um personagem chamado **${nomePersonagem}**.`);
                ...
                if (personagens.length === 0) {
                    const embedErro = criarEmbedErro("Nenhuma ficha encontrada", "Você não possui nenhuma ficha. Use `/ficha criar` para criar uma!");
                    await interaction.editReply({
                        embeds: [embedErro],
                    });
                    return;
            ...
            const personagemId = personagem._id?.toString() || personagem.id;
            ...
            const embed = criarProfileCard(personagem);
            await interaction.editReply({
                embeds: [embed],
                components: [menu],
            });
            ...
            console.error(erro);
            const embedErro = criarEmbedErro("Erro ao carregar ficha", "Não foi possível carregar sua ficha.");
            await interaction.editReply({
                embeds: [embedErro],
            });
        }
...
command.subcommand({
    ...
    async run(interaction) {
        ...
        try {
            ...
            const personagem = await CharacterRepository.findByUserAndName(interaction.user.id, nome);
            if (!personagem) {
                const embedErro = criarEmbedErro("Personagem não encontrado", `Você não possui um personagem chamado **${nome}**.`);
                await interaction.editReply({ embeds: [embedErro] });
                return;
            }
            const { ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder } = await import("discord.js");
            const charId = personagem._id || personagem.id;
            ...
            */
            ...
            console.error(erro);
            const embedErro = criarEmbedErro("Erro ao abrir editor", "Não foi possível abrir o editor de ficha.");
            await interaction.editReply({ embeds: [embedErro] });
        }
    },
});
    ...
    async run(interaction) {
        ...
        try {
            ...
            if (minhasPersonagens.length === 0) {
                const embedErro = criarEmbedErro("Nenhuma ficha encontrada", "Você ainda não possui nenhuma ficha. Use `/criar` para criar uma!");
                await interaction.editReply({ embeds: [embedErro] });
                return;
            }
            ...
            console.error(erro);
            const embedErro = criarEmbedErro("Erro ao listar fichas", "Não foi possível listar suas fichas.");
            await interaction.editReply({ embeds: [embedErro] });
        }
    },
});
command.subcommand({
    ...
    async run(interaction) {
        ...
        try {
            ...
            await CharacterRepository.update(personagem._id || personagem.id, {
                atualizadoEm: new Date(),
            });
            const embedSucesso = criarEmbedConfirmacao("✨ Ficha Ativada", `**${nome}** agora é sua ficha ativa.`);
            await interaction.editReply({ embeds: [embedSucesso] });
        }
        catch (erro) {
            console.error(erro);
            const embedErro = criarEmbedErro("Erro ao selecionar ficha", "Não foi possível marcar como ativa.");
            await interaction.editReply({ embeds: [embedErro] });
        }
    },
});
