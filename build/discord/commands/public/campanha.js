...
import { ActionRowBuilder, ApplicationCommandOptionType, ApplicationCommandType, EmbedBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, } from "discord.js";
...
createCommand({
    ...
    async run(interaction) {
        ...
        if (sub === "listar") {
                    ...
                    await interaction.editReply({
                        embeds: [
                            new EmbedBuilder()
                                .setColor("#8B00FF")
                                .setTitle("Campanhas")
                                .setDescription("Você não tem campanhas. Use `/criar` para criar uma!"),
                        ],
                    });
                    return;
                }
            ...
            const campanha = await CampaignRepository.findByMasterAndName(interaction.user.id, nome);
            if (!campanha) {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: "Mestre", value: `<@${campanha.mestre}>`, inline: true }, { name: "Membros", value: String(campanha.membros?.length || 1), inline: true })
                .setFooter({ text: `C.A.R.L.A // Campanha ${campanha._id}` });
            ...
            const modal = new ModalBuilder()
                .setCustomId(`editar_campanha_modal/${campanha._id}`)
            ...
            const ambientacaoInput = new TextInputBuilder()
                ...
                .setRequired(false)
                .setValue(campanha.ambientacao || "");
            modal.addComponents(new ActionRowBuilder().addComponents(descricaoInput), new ActionRowBuilder().addComponents(ambientacaoInput));
