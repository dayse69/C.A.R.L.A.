...
export async function bootstrap(options) {
    ...
    client.once("clientReady", async (client) => {
        ...
        await Promise.all(Array.from(app.events.getEvents("clientReady").values()).map((data) => BaseEventHandlers.handler(data, [client])));
    });
...
async function loadModules(workdir, modules = []) {
    ...
    const files = await glob([`!./discord/index.*`, `!./discord/base/**/*`, `./discord/${pattern}`, ...modules], { absolute: true, cwd: workdir });
