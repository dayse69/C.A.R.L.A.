...
import { ResponderType } from "./types.js";
export class BaseResponderHandlers {
            ...
            : interaction.isStringSelectMenu()
                ...
                : interaction.isChannelSelectMenu()
                    ...
                    : interaction.isRoleSelectMenu()
                        ? ResponderType.RoleSelect
                        : interaction.isUserSelectMenu()
                            ? ResponderType.UserSelect
                            : interaction.isMentionableSelectMenu()
                                ? ResponderType.MentionableSelect
    ...
    static async handler(interaction) {
        ...
        if (isBlock)
            return;
        await handler.data.run(interaction, params).catch((err) => {
            if (onError) {
