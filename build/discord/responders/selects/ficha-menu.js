...
// Criar menu de abas
function criarMenuAbas(personagemId, abaAtual) {
    ...
    async run(interaction) {
        ...
        try {
            ...
            if (!personagem) {
                ...
                const embedErro = criarEmbedErro("Ficha não encontrada", `Personagem não encontrado.`);
                await interaction.editReply({
                    embeds: [embedErro],
                    components: [],
                });
            ...
            const embedFunction = embedFunctions[abaEscolhida];
            const embed = embedFunction(personagem);
