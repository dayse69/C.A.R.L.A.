...
function buildAcervoModal(tipo, nome, titulo) {
    ...
    const requisitosInput = new TextInputBuilder()
        ...
        .setRequired(false);
    modal.addComponents(new ActionRowBuilder().addComponents(descInput), new ActionRowBuilder().addComponents(atributosInput), new ActionRowBuilder().addComponents(requisitosInput));
    return { modal };
}
createResponder({
    ...
    async run(interaction) {
        ...
        try {
            ...
            console.log(`🎯 [CRIAR-MENU] tipo=${tipoSelecionado}, nome=${nome}, origem=${origem}, raca=${racaSel}, classe=${classeSel}`);
            ...
            switch (tipoSelecionado) {
                case "ficha":
                    ...
                    console.log(`🎯 [CRIAR-MENU] → Modal shown for ficha (elapsed ${Date.now() - tFicha}ms)`);
                    break;
                case "campanha":
                    console.log("🎯 [CRIAR-MENU] → Case 'campanha' matched");
                    const tCamp = Date.now();
                    await handleCriarCampanha(interaction, nome);
                    console.log(`🎯 [CRIAR-MENU] → Modal shown for campanha (elapsed ${Date.now() - tCamp}ms)`);
                    break;
                case "acervo_poder": {
                    ...
                    const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                        ...
                        .addOptions({ label: "Racial", value: "racial", emoji: "🧬" }, { label: "Combate", value: "combate", emoji: "⚔️" }, { label: "Destino", value: "destino", emoji: "🎯" }, { label: "Magia", value: "magia", emoji: "🪄" }, { label: "Tormenta", value: "tormenta", emoji: "🌪️" }, { label: "Concedido", value: "concedido", emoji: "🙏" }));
                    const embed = new EmbedBuilder()
                        .setColor("#8B00FF")
                        .setTitle("✨ Criar Poder Geral")
                        .setDescription("Escolha a categoria do poder e, em seguida, preencha o modal.");
                ...
                case "acervo_classe_alternativa": {
                    ...
                    const { modal } = buildAcervoModal("acervo_classe_alternativa", nome, "🎭 Criar Classe Alternativa");
                    await interaction.showModal(modal);
                ...
                case "acervo_deus": {
                    ...
                    const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                        ...
                        .addOptions({ label: "Maiores", value: "maiores", emoji: "🌟" }, { label: "Menores", value: "menores", emoji: "✨" }, { label: "Servidores", value: "servidores", emoji: "🛡️" }));
                    const embed = new EmbedBuilder()
                        .setColor("#8B00FF")
                ...
                case "acervo_dominio":
                case "acervo_item": {
                    console.log("🎯 [CRIAR-MENU] → Case 'acervo_item/distincao/base/dominio' matched");
                    await interaction.deferUpdate();
                    const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                        .setCustomId(`criar_item_categoria_select/${nome}`)
                        .setPlaceholder("📦 Selecione a categoria do item...")
                        .addOptions({ label: "Mundanos", value: "mundanos", emoji: "🧰" }, { label: "Consumíveis", value: "consumiveis", emoji: "🧪" }, { label: "Mágicos", value: "magicos", emoji: "✨" }, { label: "Aprimorados", value: "aprimorados", emoji: "🛠️" }));
                    const embed = new EmbedBuilder()
                        .setColor("#8B00FF")
                        .setTitle("📦 Criar Item do Acervo")
                        .setDescription("Escolha a categoria do item e, em seguida, preencha o modal.");
                    await interaction.update({ content: "", embeds: [embed], components: [row] });
...
async function handleCriarFicha(interaction, nome, origem, racaSel, classeSel) {
    const modal = new ModalBuilder()
    ...
    const classeInput = new TextInputBuilder()
        ...
        .setRequired(false);
    if (classeSel)
        classeInput.setValue(classeSel);
    ...
    const descricaoInput = new TextInputBuilder()
        ...
        .setRequired(false)
        .setMaxLength(500);
    modal.addComponents(new ActionRowBuilder().addComponents(racaInput), new ActionRowBuilder().addComponents(classeInput), new ActionRowBuilder().addComponents(nivelInput), new ActionRowBuilder().addComponents(descricaoInput));
    const t = Date.now();
    await interaction.showModal(modal);
    console.log(`🎯 [CRIAR-MENU] handleCriarFicha showModal elapsed ${Date.now() - t}ms`);
}
// Handler para criar campanha
async function handleCriarCampanha(interaction, nome) {
    const modal = new ModalBuilder()
    ...
    const nivelInicialInput = new TextInputBuilder()
        ...
        .setPlaceholder("1-20 (padrão: 1)")
        .setRequired(false);
    modal.addComponents(new ActionRowBuilder().addComponents(descricaoInput), new ActionRowBuilder().addComponents(ambientacaoInput), new ActionRowBuilder().addComponents(nivelInicialInput));
    const t = Date.now();
    await interaction.showModal(modal);
