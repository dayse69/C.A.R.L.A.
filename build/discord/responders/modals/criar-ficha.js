...
import { getClassNames, getRaceNames, isValidClass, isValidRace, } from "../../../services/compendiumService.js";
import { criarPersonagem } from "../../../services/fichaService.js";
function buildSuggestionList(values, limit = 5) {
    return values
        .filter(Boolean)
        .slice(0, limit)
        .map((v) => `• ${v}`)
...
createResponder({
    ...
    async run(interaction) {
        ...
        try {
            ...
            console.log(`📝 [CRIAR-FICHA] fields: raca=${raca}, classe=${classe}, nivel=${nivel}, desc=${descricao?.substring(0, 30)}`);
            console.log(`📝 [CRIAR-FICHA] payload -> user=${interaction.user?.id} guild=${interaction.guild?.id} nome=${nome} origem=${origem} raca=${raca} classe=${classe} nivel=${nivel} desc_len=${descricao.length}`);
            const existente = await CharacterRepository.findByUserAndName(interaction.user.id, nome);
            ...
            const invalidFields = [];
            if (raca && !isValidRace(raca))
                invalidFields.push("raça");
            if (classe && !isValidClass(classe))
                invalidFields.push("classe");
            if (invalidFields.length) {
                const suggestions = [];
                if (invalidFields.includes("raça")) {
                    const races = buildSuggestionList(getRaceNames());
                    if (races)
                        suggestions.push(`Raças sugeridas:\n${races}`);
                }
                if (invalidFields.includes("classe")) {
                    const classes = buildSuggestionList(getClassNames());
                    if (classes)
                        suggestions.push(`Classes sugeridas:\n${classes}`);
                }
                await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(`Revise os campos: ${invalidFields.join(", ")}.\n` +
                            (suggestions.length ? `\n${suggestions.join("\n\n")}` : "")),
                    ],
                    ephemeral: true,
                });
            ...
            novo.origem = origem || undefined;
            novo.descricao = descricao || undefined;
            novo.categoria = "ficha_personagem";
            await CharacterRepository.create(novo);
            console.log(`[criar] ficha criada user=${interaction.user.id} nome=${nome} raca=${racaFinal} classe=${classeFinal} nivel=${nivel} backend=CharacterRepository`);
            const embed = new EmbedBuilder()
                .setColor("#8B00FF")
                .setTitle("✨ Ficha Criada!")
                .setDescription(`📜 **${nome}** criado com sucesso.\n` +
