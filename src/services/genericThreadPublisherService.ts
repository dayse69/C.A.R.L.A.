 ...
 */
function buildEntryCharacteristicsEmbeds(categoria: Categoria, entry: AcervoEntry): EmbedBuilder[] {
    ...
    const color: any = colorMap[categoria] || "#D946EF";
    ...
    const mainEmbed = new EmbedBuilder()
        .setColor(color)
        .setTitle(`${emoji} ${name}`)
        .setDescription((description as string).slice(0, 4096))
    ...
    if (additionalFields.length > 0) {
        ...
        for (const [key, value] of additionalFields) {
            ...
            const fieldValue = (Array.isArray(value) ? value.join(", ") : String(value)).slice(
                0,
                1024
            );
 ...
 */
export async function publishEntryToThread(
    channel: TextChannel,
    categoria: Categoria,
    entry: AcervoEntry,
    delayMs: number = 300
 ...
 */
export async function publishAllEntriesToThreads(
    channel: TextChannel,
    categoria: Categoria,
    entries: AcervoEntry[],
    delayMs: number = 500
    ...
    console.log(
        `[ThreadPublisher] Iniciando publicação de ${entries.length} entradas (${categoria}) em threads...`
    );
    ...
    console.log(
        `[ThreadPublisher] ✅ Publicação concluída: ${published}/${entries.length} entradas`
    );
    return published;
}
