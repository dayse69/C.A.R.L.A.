 ...
 */
function buildClassCharacteristicsEmbeds(classData: ClassData): EmbedBuilder[] {
        ...
        .addFields(
            {
                ...
                value:
                    classData.primaryAttribute || classData.atributo_primario || "Não especificado",
                inline: true,
            },
            {
    ...
    const skillsText =
        classData.initialSkills || classData.habilidades_iniciais || "Não especificadas";
 ...
 */
export async function publishClassToThread(
    channel: TextChannel,
    classData: ClassData,
    delayMs: number = 300
): Promise<void> {
 ...
 */
export async function publishAllClassesToThreads(
    channel: TextChannel,
    classes: ClassData[],
    delayMs: number = 500
): Promise<number> {
    console.log(
        `[ClassThreadPublisher] Iniciando publicação de ${classes.length} classes em threads...`
    );
    ...
    console.log(
        `[ClassThreadPublisher] ✅ Publicação concluída: ${published}/${classes.length} classes`
    );
    return published;
}
