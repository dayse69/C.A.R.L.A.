...
const racasBase = racas.filter((r: any) => r.tipo === "raca_base");
...
racasBase.slice(0, 5).forEach((r: any) => {
...
// Verificar atributos
const racasComModificadores = racas.filter(
    (r: any) => r.modificadores_atributo || r.attributeModifiers
);
console.log(
    `   ✓ Raças com modificadores de atributo: ${racasComModificadores.length}/${racas.length}`
);

// Verificar habilidades
const racasComHabilidades = racas.filter((r: any) => (r.habilidades || r.abilities)?.length > 0);
