...
function readTxtFiles(importDir: string): { file: string; content: string }[] {
...
function extractSections(content: string): ParsedSection[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isAllCapsTitle =
            /^[A-ZÁÉÍÓÚÂÊÎÔÛÇ\s\-]{3,60}$/.test(line) &&
            line.length > 2 &&
            !line.includes(":") &&
            line.split(" ").length <= 8;
...
function extractMetadata(line: string, section: ParsedSection): void {
    // Procurar padrões de Atributo Primário
    const attrMatch = line.match(
        /(?:atributo(?:\s+prim[aá]rio)?|primary\s+attr|atr\.)\s*[:=]?\s*([A-Za-záéíóúâêîôûç\s,ou]+?)(?:\s*$|[.;])/i
    );
    if (attrMatch) section.primaryAttribute = attrMatch[1].trim();

    // Procurar padrões de Pontos de Vida (PV)
    const pvMatch = line.match(/(?:pv|points?\s+of\s+life|vida)\s*[:=]?\s*(\d+)/i);
    if (pvMatch) section.pv = parseInt(pvMatch[1], 10);

    // Procurar padrões de Pontos de Mana (PM)
    const pmMatch = line.match(/(?:pm|points?\s+of\s+mana|mana)\s*[:=]?\s*(\d+)/i);
    if (pmMatch) section.pm = parseInt(pmMatch[1], 10);

    // Procurar perícias iniciais
    const skillsMatch = line.match(/(?:perícias?|skills?|inicial)\s*[:=]?\s*(.+?)(?:\s*$|[.;])/i);
    if (skillsMatch) section.initialSkills = skillsMatch[1].trim();

    // Procurar bônus de atributo (para raças)
    const bonusMatch = line.match(
        /(?:atributo\s+base|attribute\s+bonus|bônus)\s*[:=]?\s*([+\-]?\s*[A-Za-záéíóúâêîôûç,\s]+)/i
    );
    if (bonusMatch) section.attributeBonus = bonusMatch[1].trim();

    // Procurar habilidades
    const abilitiesMatch = line.match(/(?:habilidades?|abilities?)\s*[:=]?\s*(.+?)(?:\s*$|[.;])/i);
    if (abilitiesMatch) section.abilities = abilitiesMatch[1].trim();

    // Procurar categoria
    const categoryMatch = line.match(/(?:categoria|category|tipo)\s*[:=]?\s*(.+?)(?:\s*$|[.;])/i);
    if (categoryMatch) section.category = categoryMatch[1].trim();

    // Procurar raridade
    const rarityMatch = line.match(
        /(?:raridade|rarity)\s*[:=]?\s*(comum|incomum|raro|lendário|mundano|aprimado|mágico)/i
    );
    if (rarityMatch) section.rarity = rarityMatch[1].trim();

    // Procurar custo
    const costMatch = line.match(/(?:custo|cost|preço|price)\s*[:=]?\s*([\d,T$\s]+)/i);
    if (costMatch) section.cost = costMatch[1].trim();
}

function buildClassTemplatesFromSections(sections: ParsedSection[]): ClassTemplate[] {
...
function buildRaceTemplatesFromSections(sections: ParsedSection[]): RaceTemplate[] {
    const templates: RaceTemplate[] = [];
    for (const section of sections) {
        const name = section.title.replace(/\s+/g, " ").trim();
        if (name.length < 3 || /^(descrição|descriçao|details?|informações)$/i.test(name)) {
            continue;
...
function buildItemTemplatesFromSections(sections: ParsedSection[]): ItemTemplate[] {
...
function main() {
    ...
    if (type === "classes") {
        ...
        fs.writeFileSync(
            path.join(outDir, "classes.templates.json"),
        ...
        fs.writeFileSync(
            path.join(outDir, "races.templates.json"),
            JSON.stringify(output, null, 2),
            "utf-8"
        );
        console.log(
        ...
        fs.writeFileSync(
            path.join(outDir, "items.templates.json"),
            JSON.stringify(output, null, 2),
            "utf-8"
        );
        console.log(
