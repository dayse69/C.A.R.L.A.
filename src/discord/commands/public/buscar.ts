    ...
    options: [
        {
            ...
            autocomplete: async (interaction) => {
                ...
                try {
                    ...
                    const nomes: string[] = [];
                    const pushNomes = (arr: any[]) =>
                        arr?.forEach?.((x) => x?.nome && nomes.push(String(x.nome)));
                    ...
                    if (acervo.poderes_gerais) {
                        ["racial", "combate", "destino", "magia", "tormenta", "concedido"].forEach(
                            (cat) => pushNomes(acervo.poderes_gerais[cat] || [])
                        );
                    }
                    if (acervo.itens) {
                        ["mundanos", "consumiveis", "magicos", "aprimorados"].forEach((cat) =>
                            pushNomes(acervo.itens[cat] || [])
                        );
                    }
    ...
    async run(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    `Encontrados **${resultados.length}** item(s). Exibindo os primeiros ${limitados.length}.`
                )
                .addFields(fields)
