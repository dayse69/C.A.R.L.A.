...
createResponder({
    ...
    async run(interaction) {
        console.log(
            `[criar] campanha modal submit by user=${interaction.user?.id} customId=${interaction.customId}`
        );
        ...
        try {
            const existente = await CampaignRepository.findByMasterAndName(
                interaction.user.id,
                nome
            );
            if (existente) {
            ...
            console.log(
                `[criar] campanha criada user=${interaction.user.id} nome=${nome} id=${saved?._id}`
            );
