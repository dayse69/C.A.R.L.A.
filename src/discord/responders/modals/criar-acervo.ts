...
function resolveCollection(json: any, target: string): any[] | null {
    ...
    if (target.startsWith("poderes_gerais.")) {
        ...
        return (json.poderes_gerais as any)[cat];
    }
...
createResponder({
    ...
    async run(interaction) {
        console.log(
            `[criar] acervo modal submit by user=${interaction.user?.id} customId=${interaction.customId}`
        );
        ...
        const hasAllowedRole = interaction.inGuild()
            ? roleIds.some(
                  (rid: string) =>
                      ...
                      (interaction.member.roles as any).cache?.has?.(rid)
              )
            : false;

        if (!(isAllowedUser || hasPerms || hasAllowedRole)) {
            console.warn(
                `[criar] acervo denied for user=${interaction.user.id} guild=${interaction.guild?.id}`
            );
            await interaction.reply({
                content:
                    "❌ Você não tem permissão para criar/editar o Acervo. (Requer Administrador, Gerenciar Servidor, ou cargo/config autorizado)",
                ephemeral: true,
            });
            return;
        }
        ...
        // Inserção básica
        const item: any = { id: normalizeId(nome), nome, descricao };
        if (atributos) item.atributos = atributos;
        if (requisitos) item.requisitos = requisitos;
        ...
        const exists = collection.some(
            (entry: any) => normalizeId(entry?.nome || entry?.name || entry?.id || "") === item.id
        );
        if (exists) {
            await interaction.reply({
                embeds: [
