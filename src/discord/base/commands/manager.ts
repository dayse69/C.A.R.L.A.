...
type StoredAppCommandData = GenericAppCommandData &
    Required<Pick<GenericAppCommandData, "type">> & { modules?: CommandModule[] };
...
export class CommandManager {
    ...
    private readonly autocompleteRunners = new Collection<
        string,
        AutocompleteRun<string | number, boolean>
    >();
    ...
    public getHandler(type: ApplicationCommandType, ...path: (string | null)[]) {
        ...
        return (
            this.commandRunners.get(resolved) ?? this.commandRunners.get(`/${type}/${commandName}`)
        );
    }
            ...
            for (const group of groups) {
                ...
                for (const subcommand of data) {
                    this.commandRunners.set(`${path}/${group.name}/${subcommand.name}`, [
                        run,
                        group.run,
                        (subcommand as CommandModule).run,
                    ]);
                }
            }
        }
    ...
    public build() {
        return Array.from(this.collection.values()).map((raw) => {
            ...
            const buildedOptions = this.buildOptions(
                [...(options ?? []), ...this.resolveModules(modules ?? [], path, data.run)],
                path
            );
