...
interface StringOptionData<P> extends BaseOptionData, AutocompleteOptionData<string, P> {
...
interface NumberOptionData<P> extends BaseOptionData, AutocompleteOptionData<number, P> {
...
type RunInteraction<T, P> = T extends ApplicationCommandType.Message
...
type BaseAppCommandData = Omit<BaseApplicationCommandData, "dmPermission"> &
...
export type GenericAppCommandData = AppCommandData<CommandType, boolean, unknown>;

type ResolveCommandModuleData<R> = R extends void ? undefined : R;

export type SubCommandModuleData<P extends boolean, R> = Omit<BaseOptionData, "required"> & {
    group?: string;
    run(
        this: RunThis,
...
export type SubCommandGroupModuleData<P extends boolean, R, T> = Omit<
    BaseOptionData,
    "required"
> & {
...
type CommandInstanceMethods<M extends "group" | "subcommand", D extends boolean, R> = Pick<
    {
        group<T = R>(
            data: SubCommandGroupModuleData<D, R, T>
        ): CommandInstanceMethods<"subcommand", D, T>;
        subcommand(data: Omit<SubCommandModuleData<D, R>, "group">): void;
    },
    M
>;

export type CommandInstance<T extends CommandType, P extends boolean, R> = AppCommandData<T, P, R> &
