...
export type CheckRoute<R> = R extends `${string}/`
    ? "Custom ID must not end with a slash"
    : R extends `:${string}`
      ? "Custom ID must not be a parameter only"
      : R extends `${string}:`
        ? "Custom ID must not end with a colon"
        : R extends `*${string}`
          ? "Custom ID must not start with a wildcard"
          : R;
...
export type GenericResponderData = ResponderData<string, readonly ResponderType[], any, CacheType>;
