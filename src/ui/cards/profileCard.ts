 ...
 */
export function criarProfileCard(character: Character): EmbedBuilder {
    ...
    const classePrincipal =
        classes.length > 0 ? classes[0].nome : (character as any).classe || "Aventureiro";
    ...
    const criarBarra = (atual: number, maximo: number, tamanho: number = 10): string => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            `**${character.raca}** • ${classePrincipal}\n` +
                `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                `\`\`\`ansi\n` +
                `[2;36m╔═══════════════════════════════╗[0m\n` +
        ...
        .addFields(
            {
                name: "🛡️ Sobre",
                value:
                    (character as any).historia?.aparencia || "Um misterioso aventureiro de Arton",
                inline: false,
            },
            {
                name: "⚔️ Defesa",
            ...
            {
                name: "💰 Ouro",
                value: `\`${(character as any).ouro || 0} TO\``,
                inline: true,
            }
        )
        .setThumbnail("https://i.imgur.com/placeholder.png") // Adicionar avatar personalizado depois
        .setFooter({
            text: `ID: ${(character as any)._id || "unknown"} • Use o menu para navegar`,
        })
        .setTimestamp();
 ...
 */
export function criarCombateCard(character: Character): EmbedBuilder {
    const pvAtual = character.recursos?.pv?.atual ?? 0;
    const pvMaximo = character.recursos?.pv?.maximo ?? 12;
    const pmAtual = character.recursos?.pm?.atual ?? 0;
    const pmMaximo = character.recursos?.pm?.maximo ?? 10;

    const criarBarra = (atual: number, maximo: number): string => {
    ...
    return new EmbedBuilder()
        ...
        .addFields(
            ...
            },
            {
                name: "🔥 Resistências",
                value:
                    `Fortitude: \`${character.pericias?.fortitude ?? 0}\`\n` +
 ...
 */
export function criarPericiasCard(character: Character): EmbedBuilder {
    ...
    const formatarPericia = (nome: string, valor: number): string => {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            `\`\`\`ansi\n` +
                ...
                `\n` +
                periciasArray
                    .map(([nome, valor]) => formatarPericia(nome, valor as number))
                    .join("\n")
        )
