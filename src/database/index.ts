    ...
    try {
        ...
        if (useLocalDB) {
            ...
            try {
                ...
                logger.error(
                    "Falha ao conectar ao MongoDB remoto. Usando LocalDB como fallback.",
                    error
                );
                useLocalDB = true;
                await localdb.initialize();
 ...
 */
function createCollectionProxy(collectionName: any) {
    return {
 ...
 */
function createCollectionProxyWithWait(collectionName: any) {
...
export async function saveToBothDatabases(
    ...
    try {
        ...
        if (operation === "insertOne") {
            const [doc] = args;
            mongoResult = await collections[collectionName as keyof typeof collections].insertOne(
                doc
            );
            ...
            try {
                await localdb.insert(collectionName as any, id, { ...doc, _id: id });
                console.log(`[DualSave] ✅ Salvo em MongoDB e LocalDB: ${collectionName}/${id}`);
            } catch (localErr) {
                console.warn(
                    `[DualSave] ⚠️ MongoDB OK, LocalDB falhou para ${collectionName}/${id}:`,
                    localErr
                );
            }
            ...
            mongoResult = await collections[collectionName as keyof typeof collections].updateOne(
                filter,
                update
            );
            ...
            try {
                const docs = await localdb.find(collectionName as any, filter);
                if (docs.length > 0) {
                    const updateData = update.$set || update;
                    await localdb.update(collectionName as any, docs[0]._id, updateData);
                    console.log(
                        `[DualSave] ✅ Atualizado em MongoDB e LocalDB: ${collectionName}/${docs[0]._id}`
                    );
                }
            } catch (localErr) {
                console.warn(
                    `[DualSave] ⚠️ MongoDB OK, LocalDB falhou para ${collectionName}:`,
                    localErr
                );
            }
        ...
        if (operation === "insertOne") {
            ...
            await localdb.insert(collectionName as any, id, { ...doc, _id: id });
            console.log(
                `[DualSave] ✅ Salvo apenas em LocalDB (fallback): ${collectionName}/${id}`
            );
            return id;
        } else if (operation === "updateOne") {
            const [filter, update] = args;
            ...
            if (docs.length > 0) {
                ...
                await localdb.update(collectionName as any, docs[0]._id, updateData);
                console.log(
                    `[DualSave] ✅ Atualizado apenas em LocalDB (fallback): ${collectionName}/${docs[0]._id}`
                );
            }
            return "updated";
        }
